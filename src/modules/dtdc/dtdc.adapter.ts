...
import {
  ...
  buildLabelQuery,
  buildPickupBody,
  buildQuoteChargesBody,
  buildServiceabilityBody,
...
/**
 * DTDC adapter (§8.2, §9.3.4) — a DIRECT courier reached on the merchant's
 * own credentials (BYOC, INV-23): quoted prices pass through unmarked. One
 * instance per courier_account build context (courier_account_id + mode +
 * credentials version, RW-20).
 *
 ...
 *
 * Idempotent create (A1-04, INV-5): DTDC's booking is keyed by the client
 * reference (`customer_reference_number` = intent.merchantReference);
 ...
 *
 * ndrAction is DECLARED UNSUPPORTED (A1-03): DTDC exposes NDR/reattempt
 * endpoints inconsistently across its public surfaces, so v1 routes NDR
 * handling to the manual fallback note seeded in courier_capability
 * (dtdc.seed.ts) instead of guessing at an endpoint.
 *
 * Every endpoint URL, request and response mapping lives in
 * dtdc-api.map.ts with TODO(sandbox-verify) markers; this file should not
 * need to change on a sandbox pass.
 */

/** A1-03 fallback note, mirrored into courier_capability by dtdc.seed.ts. */
export const DTDC_NDR_MANUAL_FALLBACK_NOTE =
  'DTDC exposes NDR/reattempt endpoints inconsistently; handle NDR actions manually in the DTDC panel at v1 (TODO(sandbox-verify): add an endpoint if a stable one exists).';
...
export interface DtdcCallRecord {
...
export class DtdcAdapter implements CourierAdapter {
  ...
  readonly requestLog: DtdcCallRecord[] = [];
  ...
  constructor(options: DtdcAdapterOptions) {
    this.courierCode = options.courierCode ?? DTDC_COURIER_CODE;
    this.baseUrl =
  ...
  private log(rec: DtdcCallRecord): void {
    this.requestLog.push(rec);
  }

  private authHeaders(): Record<string, string> {
    return { [DTDC_AUTH_HEADER]: this.apiKey };
  }
  ...
  async getQuote(request: QuoteRequest): Promise<QuoteResponse> {
    ...
    const svcBody = await this.call('getQuote', DTDC_ENDPOINTS.serviceability, {
      body: buildServiceabilityBody({
    ...
    const quoteBody = await this.call('getQuote', DTDC_ENDPOINTS.quoteCharges, {
      body: buildQuoteChargesBody({
        originPincode: request.originPincode,
        destinationPincode: request.destinationPincode,
    ...
    return {
      ...
      total: charges.total,
      currency: 'INR',
      // DTDC rates RTO as a charge component, not a return-charge term —
      ...
      fetchedAt,
      providerQuoteRef: null, // the calculator returns no quote reference (TODO(sandbox-verify))
      capabilityFlags,
    };
  }
  ...
  async createShipment(request: CreateShipmentRequest): Promise<CreateShipmentResult> {
    ...
    try {
      const createBody = await this.call('createShipment', DTDC_ENDPOINTS.createShipment, {
  ...
  async lookupByReference(merchantReference: string): Promise<LookupByReferenceResult> {
    ...
    const body = await this.call('lookupByReference', DTDC_ENDPOINTS.tracking, {
  ...
  async cancelShipment(awb: string): Promise<CancelShipmentResult> {
    ...
    try {
      const body = await this.call('cancelShipment', DTDC_ENDPOINTS.cancel, {
  ...
  async track(awb: string): Promise<TrackEvent[]> {
    ...
    const body = await this.call('track', DTDC_ENDPOINTS.tracking, {
      query: buildTrackingQueryByWaybill(awb),
    });
    const parsed = parseTrackingResponse(body);
    if (!parsed || !parsed.awb) {
    ...
    return parsed.scans.map((scan) => ({
      ...
      // DTDC scans carry no provider event ID (TODO(sandbox-verify));
  ...
  // ------------------------------------------------------------------
  // §8.2 getLabel — courier PDF (label_mode CUSTOM_ALLOWED, §9.9.1: DTDC
  // allows custom labels; the courier PDF is still offered here)
  // ------------------------------------------------------------------
  ...
  async ndrAction(_request: NdrActionRequest): Promise<NdrActionResult> {
    ...
    // A1-03: never a silent no-op — the declared-unsupported capability
    // throws, and the seeded courier_capability row points merchants at the
    // manual fallback.
 ...
 * (`api_key`), picks the base URL by mode. The plaintext key is captured
 * inside the instance and never logged or re-emitted (§5.7 control 1,
 * INV-18).
 */
export const dtdcAdapterFactory: AdapterFactory = (ctx: AdapterBuildContext) => {
