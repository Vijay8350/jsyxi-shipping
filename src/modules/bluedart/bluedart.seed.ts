...
import { BLUEDART_COURIER_CODE, BLUEDART_NDR_FALLBACK_NOTE } from './bluedart-api.map';

/**
 * Blue Dart seed (§9.3.4 launch adapter): the global courier row, its
 ...
 *
 * Exported as data (BLUEDART_SEED) plus runBluedartSeed(pool) performing
...
export const BLUEDART_SEED = {
  courier: {
    code: BLUEDART_COURIER_CODE,
    name: 'Blue Dart',
    kind: 'DIRECT', // §9.3.4: Blue Dart is a DIRECT courier (BYOC)
    authPattern: 'KEY_PASTE', // pasted client_id + client_secret pair
  },
  ...
  credentialFields: [
    ...
    },
    {
      ...
      key: 'pickup_code',
      label: 'Customer code (registered pickup)',
      type: 'text',
      isSecret: false,
      isRequired: false,
      validationRegex: null as string | null,
  ...
  /**
   * §8.2 capabilities: Blue Dart implements 7 of 8 adapter methods.
   * ndrAction is supported = false (A1-03): Blue Dart's NDR-action APIs are
   * inconsistent at v1, so the capability carries a manual_fallback_note and
   * the adapter throws UnsupportedCapabilityError — never a silent no-op.
   */
  capabilities: ADAPTER_METHODS.map((capability) => ({
    capability,
    supported: capability !== 'ndrAction',
    manualFallbackNote: capability === 'ndrAction' ? BLUEDART_NDR_FALLBACK_NOTE : null,
  })),
  /** Services (§9.3.2): labels MUST be the courier's own PDF for Blue Dart
   *  (COURIER_PDF_REQUIRED, §9.9.1 — never custom-rendered); pricing comes
   *  from the merchant's uploaded rate card (RATE_CARD, §3.7 — BYOC,
   *  INV-23). */
  services: [
    {
      code: 'BLUEDART_APEX',
      name: 'Blue Dart Dart Apex',
      labelMode: 'COURIER_PDF_REQUIRED',
      costSource: 'RATE_CARD',
    },
    {
      code: 'BLUEDART_SURFACE',
      name: 'Blue Dart Dart Surfaceline',
      labelMode: 'COURIER_PDF_REQUIRED',
      costSource: 'RATE_CARD',
    },
  ],
  ...
  /**
   * Starter courier_status_map (§3.6). Raw statuses are Blue Dart's known
   * scan texts, stored normalized case-folded. TODO(sandbox-verify): the
   * exact raw status vocabulary against sandbox tracking payloads.
   */
  statusMap: [
    // "Shipment Booked" = booked, awaiting pickup — conservative: the
    // closest §3.6 value is PICKUP_SCHEDULED (no "booked" value).
    { rawStatus: 'shipment booked', carrierEventStatus: 'PICKUP_SCHEDULED', note: 'booked, awaiting pickup' },
    { rawStatus: 'pickup scheduled', carrierEventStatus: 'PICKUP_SCHEDULED', note: null },
    { rawStatus: 'pickup done', carrierEventStatus: 'PICKED_UP', note: null },
    ...
    // terminal state.
    { rawStatus: 'undelivered', carrierEventStatus: 'UNDELIVERED_ATTEMPT', note: 'failed delivery attempt' },
    ...
    { rawStatus: 'shipment cancelled', carrierEventStatus: 'CANCELLED_BY_COURIER', note: null },
  ] as StatusMapRow[],
};

/**
 * Idempotent upserts for every Blue Dart seed row. Safe to re-run on each
 ...
 */
export async function runBluedartSeed(pool: Pool): Promise<void> {
  ...
  try {
    ...
    const courierRes = await client.query<{ courier_id: string }>(
      ...
      [
        BLUEDART_SEED.courier.code,
        BLUEDART_SEED.courier.name,
        BLUEDART_SEED.courier.kind,
        BLUEDART_SEED.courier.authPattern,
      ],
    );
    ...
    const v = BLUEDART_SEED.serviceVersion;
    ...
    for (const m of BLUEDART_SEED.statusMap) {
      await client.query(
