...
/**
 * Shadowfax seed (§9.3.4 launch adapter): the global courier row, its
 ...
 *
 * Exported as data (SHADOWFAX_SEED) plus runShadowfaxSeed(pool) performing
...
export const SHADOWFAX_GETQUOTE_FALLBACK_NOTE =
...
export const SHADOWFAX_SEED = {
  courier: {
    code: SHADOWFAX_COURIER_CODE,
    name: 'Shadowfax',
    kind: 'DIRECT', // §9.3.4: Shadowfax is a DIRECT courier (BYOC)
  ...
  /**
   * §8.2 capabilities: Shadowfax implements 7 of 8 adapter methods.
   * getQuote is supported = false (A1-03): Shadowfax rate APIs are
   * contract-specific, and the Services are cost_source = RATE_CARD (§3.7)
   * — pricing and lane serviceability come from the merchant's rate card
   * via the §4.5 cost engine. The adapter throws
   * UnsupportedCapabilityError, never a silent no-op.
   */
  capabilities: ADAPTER_METHODS.map((capability) => ({
    capability,
    supported: capability !== 'getQuote',
    manualFallbackNote: capability === 'getQuote' ? SHADOWFAX_GETQUOTE_FALLBACK_NOTE : null,
  })),
  /** Services (§9.3.2): labels may be custom-generated for Shadowfax
  ...
  /**
   * Starter courier_status_map (§3.6). Raw statuses are Shadowfax's known
   ...
   */
  statusMap: [
    // "Created"/"Confirmed" = booked, awaiting pickup — conservative: the
    // closest §3.6 value is PICKUP_SCHEDULED (no "manifested" value).
    { rawStatus: 'created', carrierEventStatus: 'PICKUP_SCHEDULED', note: 'booked, awaiting pickup' },
    { rawStatus: 'confirmed', carrierEventStatus: 'PICKUP_SCHEDULED', note: 'booked, awaiting pickup' },
    ...
    // the reason text, not the status.
    { rawStatus: 'undelivered', carrierEventStatus: 'UNDELIVERED_ATTEMPT', note: 'NDR reason rides as reason text' },
...
/**
 * Idempotent upserts for every Shadowfax seed row. Safe to re-run on each
 ...
 */
export async function runShadowfaxSeed(pool: Pool): Promise<void> {
  ...
  try {
    ...
    const courierRes = await client.query<{ courier_id: string }>(
      ...
      [
        SHADOWFAX_SEED.courier.code,
        SHADOWFAX_SEED.courier.name,
        SHADOWFAX_SEED.courier.kind,
        SHADOWFAX_SEED.courier.authPattern,
      ],
    );
    const courierId = courierRes.rows[0].courier_id;

    for (const f of SHADOWFAX_SEED.credentialFields) {
      await client.query(
    ...
    const v = SHADOWFAX_SEED.serviceVersion;
    ...
    for (const m of SHADOWFAX_SEED.statusMap) {
      await client.query(
